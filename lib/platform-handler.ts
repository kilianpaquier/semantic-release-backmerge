...
class Bitbucket implements PlatformHandler {
    ...
    constructor(baseUrl: string, token: string, basePath?: string) {
        this.apiUrl = path.join(baseUrl, basePath ?? "/rest/api/1.0")
        this.token = token
...
class BitbucketCloud implements PlatformHandler {
    ...
    constructor(baseUrl: string, token: string, basePath?: string) {
        this.apiUrl = path.join(baseUrl, basePath ?? "/2.0")
        this.token = token
    ...
    async hasPull(owner: string, repository: string, from: string, to: string): Promise<boolean> {
        ...
        while (next !== "") {
            ...
            next = result.next // next is empty when it's the last page
...
class Gitea implements PlatformHandler {
    ...
    constructor(baseUrl: string, token: string, basePath?: string) {
        this.apiUrl = path.join(baseUrl, basePath ?? "/api/v1")
        this.token = token
...
class Github implements PlatformHandler {
    ...
    constructor(baseUrl: string, token: string, basePath?: string) {
        this.apiUrl = path.join(baseUrl, basePath ?? "")
        this.token = token
...
class Gitlab implements PlatformHandler {
    ...
    constructor(baseUrl: string, token: string, basePath?: string) {
        this.apiUrl = path.join(baseUrl, basePath ?? "/api/v4")
        this.token = token
